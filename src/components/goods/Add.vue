      ...
      <el-form :model="goodsModel" :rules="goodsRules" ref="goodsFormRef" label-position="top">
        <el-tabs tab-position="left" v-model="activeStep" :before-leave="toggleTab">
          ...
          <el-tab-pane label="商品图片" name="3">
            ...
            :on-remove="removeUpload" :headers="headersObj" :on-success="uploadSuccess">
...
export default {
  ...
  methods: {
    ...
    removeUpload (file) {
      ...
      const idx = this.goodsModel.pics.findIndex(item => item.pic === filePath)
      this.goodsModel.pics.splice(idx, 1)
      // console.log(this.goodsModel.pics)
    },
    uploadSuccess (res) {
      const pic = { pic: res.data.tmp_path }
      this.goodsModel.pics.push(pic)
    },
